...
class OpGISTLoadImage(OpBase):
    ...
    def __call__(self, sample_dict: NDict, key_in: str, key_out: str) -> NDict:
        ...
        # --- Get spacing in [X, Y, Z] ---
        spacing_xyz = np.array(img.header.get_zooms(), dtype=np.float32)  # [X, Y, Z]

        # test
        t_key = key_out + ".b_swap"
        sample_dict[t_key] = img_np.copy()

        # --- Swap axes so shortest dimension is last (on [X, Y, Z] data) ---
        # This ensures the min dimension will become Z after transpose
        img_np, spacing_xyz, swap_info = self._swap_axes_to_standard(img_np, spacing_xyz)
        # After swap: img_np is [?, ?, min], spacing_xyz is reordered accordingly

        # --- Transpose to [Z, Y, X] format (putting min first as Z) ---
        img_np = np.transpose(img_np, axes=(2, 1, 0))  # [?, ?, min] → [min, ?, ?] = [Z, Y, X]
        spacing_xyz = spacing_xyz[::-1]  # Reverse to match transpose

        # --- Save into sample_dict ---
        sample_dict[key_out] = img_np.copy()
        sample_dict["data.input.img.original_spacing"] = spacing_xyz
...
class OpClipMaskedNoNorm(OpBase):
    ...
    def __init__(
        ...
        clip_range: Tuple[float, float] = (0, 250),
        background_val: float = 0,
    ):
        super().__init__()
    ...
    def __call__(self, sample_dict: NDict, key_in: Tuple[str, str], key_out: str) -> NDict:
        ...
        # clip tumor voxels (NO min-max normalization), no clip for MRI
        tumor_voxels = ct[mask]
        # clipped = np.clip(tumor_voxels, self.clip_range[0], self.clip_range[1])

        out[mask] = tumor_voxels.astype(np.float32)
...
class OpTumorRandomRotation(OpBase):
    ...
    def __call__(self, sample_dict: NDict, key_in: Tuple[str, str], key_out: Tuple[str, str]) -> NDict:
        ...
        ct_rot = rotate(
            ...
            reshape=True, order=3, mode='constant', cval=0.0
        )
        seg_rot = rotate(
